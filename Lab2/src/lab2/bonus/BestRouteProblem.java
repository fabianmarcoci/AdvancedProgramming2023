...
public class BestRouteProblem {
    ...
    public String getRoads(int i) {
        return this.roads.get(i).getName();
    }
    ...
    * */
    ...
    public void straightWays(String LocationsOnRoads[][], String RoadsOnLocations[][]) {
        for (int i = 1; i <= roads.size(); i++) {
            System.out.print("Possible ways on the road " + LocationsOnRoads[0][i] + ": ");
            int count = 0;
            RoadsOnLocations[i-1][count++] = LocationsOnRoads[0][i];
            for(int j = 1; j <= locations.size(); j++) {
                if(LocationsOnRoads[i][j] == "true") {
                    System.out.print(LocationsOnRoads[j][0] + " ");
                    RoadsOnLocations[i-1][count++] = LocationsOnRoads[j][0];
                }
            }
            System.out.println();
        }
